...
| `TrendChart`          | `TrendDatum[]` (`ScenarioDatum & { summary? }`)                                                            | `comparison`, `variance`, `referenceLines={["PY","PL"]}`, `showValueLabels`, `onSelect` (720×360) |
| `StructureChart`      | `StructureDatum[]` = `{ label, AC?, PY?, PL?, FC?, higherIsBetter? }` — key is **`label`**, not `category` | `sort="desc"\|"asc"\|"none"`, `showComparison`, `showShare`, `variance`, `labelWidth` (600×320)   |
