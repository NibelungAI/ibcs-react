...
const rows = await page.evaluate(() => [...document.querySelectorAll(".page")].map((pg) => {
  const body = pg.querySelector(".body"), flow = pg.querySelector(".flow");
  const edge = pg.getBoundingClientRect().right - 14;
  if (pg.classList.contains("cover")) return { page: +pg.dataset.page, avail: Math.round(body.clientHeight), used: Math.round(flow.scrollHeight), wide: 0 };
  const wide = [...pg.querySelectorAll(".flow *")].filter((e) => e.getBoundingClientRect().right > edge).length;
  return { page: +pg.dataset.page, avail: Math.round(body.clientHeight),
           used: Math.round(flow.scrollHeight), wide };
}));
await browser.close();

console.table(rows.map((r) => ({ ...r, status: r.used > r.avail ? "OVERFLOW" : (r.wide ? "too wide" : "ok"),
...
if (bad.length) {
  ...
  for (const r of bad) {
    const parts = await p2.evaluate((n) => [...document.querySelector(`[data-page="${n}"] .flow`).children]
      .map((c) => ({ el: c.tagName.toLowerCase() + (c.className ? "." + c.className.split(" ").join(".") : ""),
                     h: Math.round(c.getBoundingClientRect().height) })), r.page);
