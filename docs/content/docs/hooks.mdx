...
Eased progress from `0` to `1`, played once on mount and replayed whenever
`key` changes. Multiply a bar height or a path length by it to drive an
...
identity, so an inline `data={[…]}` literal does not restart the entrance on
every parent render.
...
function GrowingBar({ data, fullHeight, base }) {
  // Replays on a real data change, not on every render.
  const p = useMountGrow(600, 0, data);
  return <rect height={fullHeight * p} y={base - fullHeight * p} />;
}
```

**When to use:** a one-shot entrance for a chart you draw yourself, or a replay
tied to a genuine data change.
...
Pair it with the animation hooks so figures tween to each tick rather than
jumping.
