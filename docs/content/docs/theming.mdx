...
- `tokenPresets` — a `Record<string, IbcsTokens>` holding all eight under the
  names `"Default"`, `"Ocean"`, `"Azure"`, `"Green / Red"`, `"Vivid"`,
  `"CVD-safe"`, `"Mono / print"` and `"Dark"` — ready to drive a theme
  switcher.
...
// …or pick one by name from the registry (handy for a theme switcher):
